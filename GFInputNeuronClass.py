...
#This is a class that is made up of information from NeuPrint queries
#It is meant to mimic the classes of the catmaid code for the GF input project
#Work in progress
class GFinputNeuron(object):
    ...
    def getSynapsesByBranch(self):
        ...
        #sum = len(synDict['medial']) + len(synDict['descending']) + len(synDict['lateral']) + len(synDict['anterior']) + len(synDict['soma'])
        #if sum != self.GF1synapseCount:
           # print(self)
           # print(sum)
           # print(self.GF1synapseCount)
           # raise ValueError("Branch inputs does not match GF Synapse Count")
...
#bodyDict is a dictonary of body ids and the values are the input type
#obtained from csv from google drive
#needs to query the info not present in the dictioanry
    #query inclues gf synapse, instance, status, name --- all things that are subject to change in neutu
